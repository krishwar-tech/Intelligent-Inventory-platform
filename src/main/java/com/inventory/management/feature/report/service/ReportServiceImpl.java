package com.inventory.management.feature.report.service;
...
public class ReportServiceImpl implements ReportService {
    ...
    public Map<String, Object> dashboard() {
        ...
        log.info(
                "Dashboard analytics loading started");
        ...
        List<Map<String, Object>> lowStockTable =
                new ArrayList<>();
        ...
        for (SaleItem item : items) {
            ...
            productSales.put(
                    name,
                    productSales.getOrDefault(name, 0) + qty);
        }
        ...
        for (Product p : products) {
            ...
            categoryMap.put(
                    cat,
                    categoryMap.getOrDefault(cat, 0)
                            +
                            (p.getStock() == null
                                    ? 0
                                    : p.getStock()));
        ...
        revenueTrend.add(
                createTrend("Mon", 1200));

        revenueTrend.add(
                createTrend("Tue", 1800));

        revenueTrend.add(
                createTrend("Wed", 900));

        revenueTrend.add(
                createTrend("Thu", 2200));

        revenueTrend.add(
                createTrend("Fri", revenue));
        ...
        List<Map<String, Object>> inventoryCategoryValue =
                categoryMap.entrySet()
                        .stream()
                        .map(e -> {
        ...
        for (Sale s : allSales) {
            ...
            m.put(
                    "paymentMode",
                    s.getPaymentMode() != null
                            ? s.getPaymentMode().name()
                            : "N/A");
        ...
        log.info(
                "Sales report generated | Transactions : {}",
                allSales.size());
        ...
        List<Procurement> allProcurements =
                procurementRepo.findByTenant_Id(
                        TenantContext.getTenantId());
        ...
        map.put(
                "totalPurchase",
                allProcurements.stream()
                        .mapToDouble(p ->
                                p.getTotalCost() == null
                                        ? 0
                                        : p.getTotalCost().doubleValue())
                        .sum());

        log.info(
                "Purchase report generated | Records : {}",
                allProcurements.size());
    ...
    private Map<String, Object> createTrend(
            String name,
            double value) {
